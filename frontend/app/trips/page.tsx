...
export default function TripsDashboardPage() {
  ...
  const fetchTripList = async () => {
    ...
    try {
      ...
      setError(
        err?.message ||
          "Could not load trips from PostgreSQL database. Make sure FastAPI backend is running on http://localhost:8000."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  // Compute filtered and sorted trips
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold bg-sky-500/20 text-sky-300 border border-sky-400/30 mb-3">
                <FolderOpen className="w-3.5 h-3.5" /> Persistent Database History
              </div>
              <h1 className="text-2xl sm:text-4xl font-extrabold tracking-tight text-white flex items-center gap-3">
                <span>Trip History Dashboard</span>
              </h1>
              <p className="text-sm text-slate-300 mt-2 max-w-2xl">
                Revisit, compare, and explore all travel itineraries generated by Amazon Bedrock AI and saved to PostgreSQL.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                disabled={isLoading}
                className="p-3 rounded-2xl bg-white/10 hover:bg-white/20 text-slate-200 border border-white/15 transition-all"
                title="Refresh trips"
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-4 sm:p-5 shadow-xs mb-8 space-y-4">
          <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
            ...
            {/* Sort Dropdown (Slide 17 Core Challenge) */}
        ...
        {/* Empty State (Slide 14 - Friendly Guidance) */}
        {!isLoading && !error && trips.length === 0 && (
          <div className="bg-white rounded-3xl border border-slate-200 p-10 sm:p-16 text-center max-w-2xl mx-auto shadow-sm my-8">
            ...
            <p className="text-slate-500 text-sm max-w-md mx-auto mb-8 leading-relaxed">
              Create your first AI-generated itinerary with Amazon Bedrock. Your customized travel plans will appear here automatically.
            </p>
            <Link
              href="/"
        ...
        {/* Trips Grid View (Slide 11 / Part 4) */}
        {!isLoading && !error && paginatedTrips.length > 0 && (
          <div className="space-y-8">
            <div className="flex items-center justify-between">
              <div className="text-xs font-semibold text-slate-500">
                Found <span className="text-slate-900 font-bold">{filteredAndSortedTrips.length}</span> itineraries
              </div>
            ...
            {/* Pagination Component (Bonus Feature) */}
            <Pagination
